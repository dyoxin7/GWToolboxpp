...
void ChatCommands::SkillToUse::AddSkillToUse(uint32_t skill)
{
    if (skill < 1 || skill > 8)
        return;
    auto i = std::find(slots.begin(), slots.end(), skill - 1);
    if (i == slots.end()) {
        slots.push_front(skill - 1);
    }
}
...
void ChatCommands::CmdUseSkill(const wchar_t*, const int argc, const LPWSTR* argv)
{
    ...
    for (int i = argc - 1; i > 0; --i) {
        ...
        skill_to_use.AddSkillToUse(num);
    }
